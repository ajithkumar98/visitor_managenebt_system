...
<?php
 	$query2="SELECT * FROM visitor where id=$id";
	...
	<tr><td>ID</td> <td>:</td> <td align="center"><?php echo $info['id']?></td></tr>
		...
		<tr><td><label>Check-in time</label></td> <td>:</td> <td align="center"> <form method="get" action="checkout.php">
		
			<input type="time" name="checkouts">
			<input type="hidden" name="checkout"  value="<?php echo $_GET['checkout']; ?>">
			<input type="submit" id="button" name="submit5">
		</form></td> </tr>
		</table>
 		</div>
 	<!-- </div> -->
